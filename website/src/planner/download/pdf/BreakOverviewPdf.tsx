...
export const BreakOverviewPdf = ({ trackStreets, intl }: Props) => {
    ...
    if (breaks.length === 0) {
        return (
            <View>
                <Text style={pdfStyles.bold}>{intl.formatMessage({ id: 'msg.breakPoints' })}</Text>
        ...
        <View>
            ...
            <View style={pdfStyles.table}>
                <View style={[pdfStyles.row, pdfStyles.bold, pdfStyles.header]}>
                    ...
                    <Text style={pdfStyles.col2}>{intl.formatMessage({ id: 'msg.breakStart' })} </Text>
                    <Text style={pdfStyles.col3}>{intl.formatMessage({ id: 'msg.breakLength' })} </Text>
                </View>
                {...breaks.map((breakWayPoint, index) => {
                    return (
                        <View key={index} style={pdfStyles.row} wrap={false}>
                            ...
                            <Text style={pdfStyles.col2}>{formatTimeOnly(breakWayPoint.frontArrival)}</Text>
